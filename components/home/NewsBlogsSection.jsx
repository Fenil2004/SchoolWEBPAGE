...
/**
 * NewsBlogsSection Component
 * 
 * Showcases latest News & Educational Articles on the Home Page.
 * Controlled via Admin Panel (`/api/blogs`).
 */
    ...
    <section className="py-20 bg-slate-50 relative overflow-hidden border-t border-slate-100">
      <div className="max-w-7xl mx-auto px-4 relative z-10 space-y-12">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 bg-brand-teal-light text-brand-teal rounded-full text-xs font-bold uppercase tracking-wider">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {blogs.map((post, idx) => (
            ...
            >
              <Card className="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-card-hover transition-all duration-300 flex flex-col justify-between h-full group">
                <div>
                  {post.image ? (
                  ...
                  <CardContent className="p-6 space-y-3">
                    <h3 className="font-bold text-slate-900 text-base md:text-lg group-hover:text-brand-teal transition-colors leading-snug line-clamp-2">
                      {post.title}
                    </h3>
